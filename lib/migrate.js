...
const sortActions = function(actions)
{
    ...
    actions.sort((a, b) => {
        ...
        // orderedActionTypes are equal, see depends field
        if (a.depends.length === 0 && b.depends.length > 0)
            return -1; // a < b
        if (b.depends.length === 0 && a.depends.length > 0)
            return 1; // b < a
        
        if (b.depends.indexOf(a.tableName) !== -1)
            return -1; // a < b (b depends a)
        
        if (a.depends.indexOf(b.tableName) !== -1)
            return 1; // b < a (a depends b)
...
let resUp =`{ fn: "createTable", params: [
    ...
    ${JSON.stringify(action.options)}
                ...
                consoleOut.push(`createTable "${action.tableName}"`);
            }
            break;
...
module.exports = {
    ...
    {
        migrationCommands = migrationCommands.slice(this.pos);
        Async.eachSeries(migrationCommands, (record, cb) => {
            console.log("["+this.pos+"] execute: "+record.fn);
            queryInterface[record.fn].apply(queryInterface, record.params).then(() => {
                console.log("OK");
                this.pos++;
                cb();
            }).catch((err) => {
                console.log("Error at pos #"+this.pos, record);
                cb(err);
            });
        },
        (err) => {
...
const saveMigration = function(migrationsDir = './migrations', modelsDir = './models', name = '', comment = '')
{
    // current state
    const currentState = {
        tables: {}
    ...
    _.each(migration.consoleOut, (v) => { log ("[Actions] "+v) });
    ...
    else
    {
        ...
        fs.writeFileSync(path.join(migrationsDir, '_current.json') );
        
        // write migration to file
        writeMigration(currentState.revision, 
...
module.exports = { saveMigration, writeMigration, getMigration, sortActions, parseDifference, reverseModels };
